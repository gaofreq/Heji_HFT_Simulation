			...
			#### ~~~ <BUY TYPE 4>: the case of joining the market ~~~ ####
				...
				if not np.isnan(oneOrder.queueFrontJudge):
					...
					if mktVolume>0:
						...
						if signals.preAsk == 0:
							meanAsk = currentMkt.ask
						elif signals.preAsk - oneOrder.limitPrice < eps:
							meanAsk = currentMkt.ask 
						else:                             
							meanAsk = min(signals.preAsk,currentMkt.ask)    
						bidVolume = max(min((meanAsk-currentMkt.vwap)/(meanAsk-oneOrder.limitPrice+0.01),1)*mktVolume,0.0)
                                                        
						bidVolumeBefore = 0
						if oneOrder.limitPrice - signals.preBid < -eps:
							bidVolumeBefore = bidVolumeBefore + signals.preBz
						if oneOrder.limitPrice - signals.preBid1 < -eps:
							bidVolumeBefore = bidVolumeBefore + signals.preBz1
						if oneOrder.limitPrice - signals.preBid2 < -eps:
							bidVolumeBefore = bidVolumeBefore + signals.preBz2
						if oneOrder.limitPrice - signals.preBid3 < -eps:
							bidVolumeBefore = bidVolumeBefore + signals.preBz3
						if oneOrder.limitPrice - signals.preBid4 < -eps:
							bidVolumeBefore = bidVolumeBefore + signals.preBz4                             
						bidVolume = bidVolume - bidVolumeBefore

						bidVolume = round(max(bidVolume,0))
			...
			#### ~~~ <BUY TYPE 4>: the case of joining the market ~~~ ####
				...
				if not np.isnan(oneOrder.queueFrontJudge):
					...
					if mktVolume>0:
						...
						if signals.preBid == 0:
							meanBid = currentMkt.bid
						elif oneOrder.limitPrice - signals.preBid < eps:
							meanBid = currentMkt.bid                      
						else:                            
							meanBid = max(signals.preBid,currentMkt.bid)                        
						askVolume = max(min((currentMkt.vwap-meanBid)/(oneOrder.limitPrice-meanBid+0.01),1)*mktVolume,0.0)

						askVolumeBefore = 0
						if oneOrder.limitPrice - signals.preAsk > eps:
							askVolumeBefore = askVolumeBefore + signals.preAz
						if oneOrder.limitPrice - signals.preAsk1 > eps:
							askVolumeBefore = askVolumeBefore + signals.preAz1
						if oneOrder.limitPrice - signals.preAsk2 > eps:
							askVolumeBefore = askVolumeBefore + signals.preAz2
						if oneOrder.limitPrice - signals.preAsk3 > eps:
							askVolumeBefore = askVolumeBefore + signals.preAz3
						if oneOrder.limitPrice - signals.preAsk4 > eps:
							askVolumeBefore = askVolumeBefore + signals.preAz4                             
						askVolume = askVolume - askVolumeBefore                            

						## further update bidVolume



						askVolume = round(max(askVolume,0))
